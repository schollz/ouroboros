...
function Looper:init()
  ...
  self.arp_options = {
    {0, 1, 2, 4, 6}, {1, 2, 3, 4, 8}, {2, 4, 6, 8, 12}, {4, 6, 8, 16}, {6, 8, 12, 24}, {8, 16, 24, 32}
  }
  self.buttons = {}
  for i = 1, 6 do
    table.insert(self.buttons, {})
    for j = 1, 6 do table.insert(self.buttons[i], false) end
  end
  self.waveforms = {}
  for i = 1, 8 do table.insert(self.waveforms, waveform_:new{id=self.id, loop=i}) end

  local do_set_crow = function()
    if self.id == 1 then
      crow.output[2].action = string.format("ar( %2.3f, %2.3f, 7)", params:get(self.id .. "attack") / 1000,
                                            params:get(self.id .. "release"))
    else
      crow.output[4].action = string.format("adsr( %2.3f,1,7, %2.3f)", params:get(self.id .. "attack") / 1000,
                                            params:get(self.id .. "release"))
    end
  end
  local params_menu = {
  ...
  local params_menu2 = {
    {
      ...
      exp=false,
      div=1,
      default=self.id == 1 and 10 or 100,
      unit="ms",
      action=do_set_crow
    }, {
      id="release",
      ...
      exp=false,
      div=0.02,
      default=self.id == 1 and 0.5 or 0.5,
      unit="s",
      action=do_set_crow
    }
  }
  ...
  for loop = 1, 8 do
    for _, pram in ipairs(params_menu) do
      ...
      params:set_action(pid, function(x)
        if pram.values ~= nil then x = pram.values[x] end
  ...
  for _, pram in ipairs(params_menu2) do
    ...
    params:set_action(pid, function(x)
      if pram.values ~= nil then x = pram.values[x] end
      if pram.action ~= nil then pram.action(x) end
    end)
  end
  ...
  params:add_option(self.id .. "midi_device", "midi device", midi_device_names, #midi_device_names > 1 and 2 or 1)
  ...
  params:set_action(self.id .. "note_pressing", function(x)
    if x == 1 then
      for r = 1, 6 do for c = 1, 6 do if not self.buttons[r][c] then self:note_grid_off(r, c) end end end
    else
      -- send a note off for all current notes
      for _, v in ipairs(self.notes_on) do self:note_grid_off(v[1], v[2]) end
    end
  end)
...
function Looper:clock_loops()
  if self.rec_loops > 0 then self.rec_loops = self.rec_loops - 1 end
  if self.rec_loops == 0 then self:rec_queue_down() end
end
...
function Looper:emit_note()
  local num_notes_on = #self.notes_on
  if num_notes_on == 0 then do return end end
  -- check if toggle mode 
  if params:get(self.id .. "note_pressing") == 1 then do return end end
  ...
  if self.id == 2 then note = params:get(self.id .. "hold_change") == 1 and chords1[clock_chord].m[x[1]][x[2]] or x[3] end
  self.note_location_playing = {x[1], x[2]}
  self:note_on(note)
end

function Looper:is_note_playing(i, j)
  if self.note_location_playing == nil then do return end end
  return self.note_location_playing[1] == i and self.note_location_playing[2] == j
end

function Looper:is_note_on(i, j)
  for _, v in ipairs(self.notes_on) do if v[1] == i and v[2] == j then do return true end end end
  return false
end
...
function Looper:note_on(note)
  ...
  for k, v in pairs(self.notes_turned_on) do
    print(string.format("[looper %d] note_off %d", self.id, k))
    self:note_off(k)
  end
  ...
  if params:get(self.id .. "midi_device") > 1 then
    -- velocity should scale the set volume from -96,6 to 0,127
    local velocity =
        util.round(util.linlin(1, 8, 10, 127, params:get(self.id .. "db" .. params:get(self.id .. "loop"))))
    print(velocity, params:get(self.id .. "db" .. params:get(self.id .. "loop")))
    midi_device[params:string(self.id .. "midi_device")]:note_on(note, velocity, params:get(self.id .. "midi_channel"))
  end
...
function Looper:note_off(note)
  ...
  if params:get(self.id .. "midi_device") > 1 then
    print(string.format("[midi %d] note_off %d", self.id, note))
    clock.run(function()
      clock.sleep(0.05)
      midi_device[params:string(self.id .. "midi_device")]:note_off(note, 0, params:get(self.id .. "midi_channel"))
...
function Looper:notes_off()
  ...
  self.note_location_playing = nil
  if self.id == 2 then crow.output[4](false) end
  -- turn off all midi notes that are on 
  for k, v in pairs(self.notes_turned_on) do self:note_off(k) end
end
...
function Looper:note_grid_on(r, c)
  ...
  if self.id == 2 then note = chords1[clock_chord].m[r][c] end
  print(r, c, note)
  print(string.format("[looper %d] note_grid_on %d,%d on: %d", self.id, r, c, note))
  if #self.notes_on == 0 then self:note_on(note) end
...
function Looper:note_grid_off(r, c)
  ...
  local j = 0
  for i, v in ipairs(self.notes_on) do if v[1] == r and v[2] == c then j = i end end
  if j > 0 then table.remove(self.notes_on, j) end
  if next(self.notes_on) == nil then self:notes_off() end
end

function Looper:rec_queue_up(x)
  print(string.format("[looper %d] rec_queue_up %d", self.id, x))
  -- don't queue up twice
  for _, v in ipairs(self.rec_queue) do if v == x then do return end end end
  table.insert(self.rec_queue, x)
  print(string.format("[looper %d] queued %d", self.id, x))
end

function Looper:is_in_rec_queue(i)
  for _, v in ipairs(self.rec_queue) do if v == i then do return true end end end
  do return false end
end
...
function Looper:rec_queue_down()
  ...
  engine.record((self.id == 1 and 0 or 8) + x, beats_total * clock.get_beat_sec(), self.id == 1 and 0 or 1,
                "/home/we/dust/audio/ouroboros/" .. os.date('%Y-%m-%d-%H%M%S') .. "_" .. self.id .. ".wav")
...
function Looper:redraw()
  ...
  for i = 1, 8 do self.waveforms[i]:redraw((self.id - 1) * 64, i * 8, 4) end
